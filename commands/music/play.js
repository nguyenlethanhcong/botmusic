...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (!voiceChannel)
            return message.channel.send("You need to be in a voice channel to play music.").then(m => m.delete({ timeout: 3000 }));

        const permissions = voiceChannel.permissionsFor(bot.user);
        if (!permissions.has("CONNECT"))
            return message.channel.send("I cannot connect to your voice channel, make sure I have permission to!").then(m => m.delete({ timeout: 3000 }));

        if (!permissions.has("SPEAK"))
            return message.channel.send("I cannot connect to your voice channel, make sure I have permission to!").then(m => m.delete({ timeout: 3000 }));

        if (!args[0])
            return message.channel.send("Please provide a song name or link to search.").then(m => m.delete({ timeout: 3000 }));
